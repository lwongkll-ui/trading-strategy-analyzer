...
def generate() -> dict:
    ...
    prompt = build_prompt()
    # POE does not reliably honour max_tokens (a 4096 cap returned 5500 and the
    # JSON came back truncated), so ask twice: full, then explicitly terse.
    for attempt, extra in enumerate((
        "",
        "IMPORTANT: your previous reply was cut off. Keep every profile to 2 "
        "sentences and every bullet to one line so the JSON closes completely.",
    )):
        ...
        u = resp.usage
        if u:
            print(f"  attempt {attempt+1}: model={resp.model} in={u.prompt_tokens} out={u.completion_tokens}")
        try:
            return _extract_json(text)
        except Exception as e:
            print(f"  attempt {attempt+1} unusable ({type(e).__name__}: {str(e)[:80]})")
    raise SystemExit(
        "ERROR: narrative generation failed twice (truncated/invalid JSON). "
        "narrative_data.py left UNCHANGED - the report will flag it as stale."
    )
...
def main():
    ...
    parts = [
        ...
        f"NARRATIVE_AS_OF = {json.dumps(as_of)}",
